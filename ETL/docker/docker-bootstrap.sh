...
if [ "$CYPRESS_CONFIG" == "true" ]; then
    ...
    export DATABASE_DB=ads_schema
    export SUPERSET__SQLALCHEMY_DATABASE_URI=mysql+pymysql://root:root@mariadb:3306/ads_schema?charset=utf8mb4
    # export SUPERSET__SQLALCHEMY_DATABASE_URI=mariadb+mariadbconnector://root:root@mariadb:3306/ads_schema
    PORT=8089
fi
if [ "$(whoami)" = "root" ]; then
    # older images may not have the postgres dev requirements installed
    echo "Installing MariaDB requirements"
